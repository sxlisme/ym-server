...
app.all('/*', function(req, res) {
    ...
    if (req.url === '/') { //羊毛首页       
        ...
        req.on('end', function() {
            ...
            if (reqParm) {
                ...
                doSql.addTask(res,reqParm.params); 
            } else {               
        ...
        req.on('end', function() {
            ...
            if (reqParm) {
                 try{
                    reqParm = JSON.parse(reqParm); 
                    doSql.queryTaskTotal(res,reqParm.params); 
                }catch(e){                    
                    res.json({ code: 500, msg: '参数有误!' });
                    //return;
                };              
    ...
    }else if (req.url === '/tasks/getYangmaoUserList') { //yangmao 获取用户列表
        doSql.queryYangmaoUser(res); 
